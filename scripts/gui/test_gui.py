...
funcionalidad centrado de evento.
"""
import os
import sys
...
def toggle_centrar():
    global centrar_mode, selected_line, canvas
    ...
    if centrar_mode:
        ...
        # eliminar línea auxiliar si existe
        if selected_line:
...
# Evento clic para centrar un punto
def on_click(event):
    global selected_line
    if not centrar_mode or event.inaxes is None:
        return
    x_click = event.xdata
    # eliminar línea previa
    if selected_line:
        selected_line.remove()
    # dibujar nueva línea auxiliar
    selected_line = ax.axvline(x_click, color='b', linestyle='--')
    canvas.draw()
    # calcular delta respecto al centro
    dur = float(spin_duracion.get())
    center = dur / 2.0
    delta = x_click - center
    # actualizar campo desplazamiento
    entry_shift.delete(0, tk.END)
    entry_shift.insert(0, f"{delta:.3f}")
    # desactivar modo centrar (sin redibujar aún)
    toggle_centrar()
...
def abrir_archivo():
    ...
    try:
        ...
        entry_hora.insert(0, st.datetime.strftime('%H:%M:%S'))
        lbl_fecha.config(text=f"Fecha: {date}   Inicio: {st.datetime.strftime('%H:%M:%S')}   Fin: {et.datetime.strftime('%H:%M:%S')}")
        entry_shift.delete(0, tk.END)
...
def previsualizar():
    global fig, ax, canvas, selected_line, centrar_mode
    ...
    try:
        ...
        messagebox.showerror("Error", "Formato de hora inicio inválido.")
        return
    try:
        dur = float(spin_duracion.get())
    ...
    entry_shift.delete(0, tk.END)
    entry_shift.insert(0, "0")
    # eliminar línea auxiliar si quedase
    if selected_line:
        selected_line.remove()
        selected_line = None
    ...
    # incrustar canvas y enlazar eventos
...
tk.OptionMenu(frame_param, channel_var, "ENT", "ENR", "ENV").grid(row=1, column=3, padx=5, sticky='w')
...
btn_centrar = tk.Button(frame_actions, text="Centrar: OFF", command=toggle_centrar)
btn_centrar.pack(side='left', padx=10)
